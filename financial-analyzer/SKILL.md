...
- 在任何指标、阈值或公式落表前，先查 `financial-analyzer/knowledge_base.json` 和最近的 adoption logs；skill 只定义流程，不固化经验。
- 新发现的稳定口径、例外规则和公式增量，先写入 `financial-analyzer/knowledge_base.json`，再通过 adoption log 落地，不回写到 skill 文本。
- 若知识库暂时缺口，先按章节证据和显式公式完成可验证计算，再在复核后补充知识库。

## 分析维度拆分

- 财务分析维度由 `references/analysis_dimension_contract.md` 单独约束，必须覆盖 target case 的样本维度和知识库维度，不能只做章节摘要。
- Excel 正式化由 `references/excel_formalization_contract.md` 单独约束，必须和分析维度分开执行，不能把模板导出当成分析完成。
- 当两者冲突时，优先满足证据闭环和维度覆盖，再考虑版式美化。
...
7. 先完整阅读最小中间产物，至少包括：
  ...
  - 现有 `financial-analyzer/knowledge_base.json` 和最近的 adoption logs，优先确认其中已有的指标定义、公式口径和特殊判定
...
9. 这是整个流程中最关键的一步：逐章阅读 `chapter_records` 中的章节原文与结构化要素，边读边写 `chapter_review_ledger.jsonl`，逐章提炼：
    ...
    - 风险判断
    - 对正式知识库的增量或修正
10. 在完成逐章阅读之后，先把附注发现整理成“发现池”，至少包含：章节原文证据、抽取到的附注数据、知识库中可复用的公式口径、需要计算的派生指标、风险信号和待归类事项。
11. 依据 `references/analysis_dimension_contract.md` 对发现池进行维度归类，把各章节发现按总览、债务、流动性、资产质量、现金流、管理层支持、资本结构、专题修正等维度重新编排；正式 Excel 的 sheet 设计必须服务于这些归类结果，而不是只复述章节顺序。
12. 在归类完成后，再进入 `references/excel_formalization_contract.md` 约束下的 Excel 正式化阶段：由 Skill 基于 `chapter_text` / `chapter_text_cleaned`、`chapter_review_ledger.jsonl`、发现池和既有知识库中的公式口径，在当前 `run_dir` 内编写一次性临时生成脚本。脚本必须显式使用刚刚抽取和加工过的数据及公式，先生成保留公式关系的 `financial_output_formula.xlsx`。
13. 公式版 workbook 的设计应体现完整 Excel 工作流，常见 sheet 可包括但不限于：`00_overview`、`01_kpi_dashboard`、`02_financial_summary`、`03_debt_profile`、`04_liquidity_and_covenants`、`05_asset_quality`、`06_cash_flow_bridge`、`07_notes_map`、`08_management_support`、`09_capital_structure`、`99_evidence_index`；具体 sheet 组合可因案例调整，但必须覆盖“总览、关键指标、债务、流动性、资产质量、现金流、附注映射、管理层支持、资本结构、证据索引”这些功能面。
14. 在核对公式版口径、公式关系、sheet 组织和展示效果之后，再由 `financial_output_formula.xlsx` 派生最终 `financial_output.xlsx`；正式版生成后必须复核核心页是否存在空值、错位、单位混写、来源链路丢失或公式被硬编码替代。
15. 由已完成的 `financial_output.xlsx`、`chapter_review_ledger.jsonl` 和既有知识库，完成正式 `analysis_report.md` 写作。`final_data.json` 和 `soul_export_payload.json` 只有在下游显式需要时才作为兼容产物生成，不属于主线完成标准。
16. 最后才做知识库净化：先和现有知识库比对，再通过 adoption log 写入正式 `financial-analyzer/knowledge_base.json`。
...
- 正式知识库、adoption log 和批次运行状态都应落在项目 `runtime/` 下，而知识库主文件本身位于 `financial-analyzer/knowledge_base.json`，不是落在 skill 安装目录里。
...
- `financial_output.xlsx`
- `analysis_report.md`
- 对 `financial-analyzer/knowledge_base.json` 的优化（通过 adoption log 落地）
