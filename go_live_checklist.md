...
1. 本案必须覆盖知识库里与主体、行业和章节主题相关的既有分析范畴。
2. 本案必须覆盖全部已抽取的主章节，不能只覆盖其中一部分章节。
3. 如果知识库覆盖不全或章节覆盖不全，其他格式、性能或版式检查都不能替代这两个门禁。
...
- 既有知识库中与本案相关的分析范畴已经被显式回顾并落在工作记录中。
- 本案所有已抽取的主章节都已经在 review ledger 中出现，且状态可以解释。
- 章节目录与正式 workpaper 之间没有明显漏章。

通过标准：

- 能明确说明本次分析覆盖了哪些知识库范畴、哪些章节、哪些未识别部分。
- 不存在“知识库没看但直接下结论”或“章节没覆盖但直接收口”的情况。

失败动作：

- 先补齐知识库回顾和章节覆盖，再继续后续门禁。
- 如果出现漏章或知识库覆盖盲区，立即停止上线。
...
12. 知识库覆盖或章节覆盖未通过，但仍试图以 Excel 外观或脚本产物放行。
